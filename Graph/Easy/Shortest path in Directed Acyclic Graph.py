...
class Solution:
    ...
    def dfs(self, source, topo_sort, visited):
        visited.add(source)
        
        for neighbor in self.graph[source]:
            if neighbor[0] not in visited:
                self.dfs(neighbor[0], topo_sort, visited)
        topo_sort.append(source)
        
    def bfs(self, topo_sort, dist):
        queue = deque(topo_sort)
        dist[0] = 0
        while queue:
            node = queue.popleft()
    ...
    def shortestPath(self, n : int, m : int, edges : List[List[int]]) -> List[int]:
        ...
        for idx in range(n):
            if idx not in visited:
                self.dfs(idx, topo_sort, visited)
                
        topo_sort = topo_sort[::-1]
        dist = [float('inf')] * n
        self.bfs(topo_sort, dist)
