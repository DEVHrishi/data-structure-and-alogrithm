...
SELECT CANDIDATE_ID FROM CANDIDATES WHERE YEARS_OF_EXP >= 2 AND INTERVIEW_ID IN (SELECT INTERVIEW_ID FROM ROUNDS GROUP BY INTERVIEW_ID HAVING SUM(SCORE) > 15);

# Time:  O(m + n)
# Space: O(m + n)

SELECT candidate_id FROM Candidates as a INNER JOIN Rounds as b ON a.interview_id = b.interview_id WHERE years_of_exp >= 2 GROUP BY candidate_id HAVING SUM(score) > 15 ORDER BY NULL;